#!/usr/bin/env bash
# =============================================================================
# DEVELOPMENT ONLY. Fills a fresh install with one plausible household, so the
# app can be looked at with real shapes in it rather than empty states.
#
...
#
# It refuses to run against anything that is not a local, development-mode
# server — two independent checks, because inventing a family's finances inside
# somebody's real records would be a difficult thing to explain and a worse one
# to undo. The server is asked what it thinks it is; a host that reports
# `production` is refused even if it is called localhost.
# =============================================================================
set -euo pipefail
...
HH=$(post "$A" /api/v1/households \
  '{"name":"Koduri","mode":"family","defaultVisibility":"private","displayName":"Ishwarya"}')
HID=$(echo "$HH" | j "['id']"); ISH=$(echo "$HH" | j "['myMemberId']")

RAVI=$(post "$A" "/api/v1/households/$HID/members" '{"displayName":"Ravi","relationship":"spouse"}' | j "['id']")
post "$A" "/api/v1/households/$HID/members" \
  '{"displayName":"Aarav","relationship":"child","dateOfBirth":"2015-04-02"}' >/dev/null
INV=$(post "$A" "/api/v1/households/$HID/invitations" \
  "{\"memberId\":\"$RAVI\",\"phone\":\"$PHONE_B\",\"role\":\"admin\"}" | j "['token']")
post "$B" /api/v1/invitations/accept "{\"token\":\"$INV\"}" >/dev/null

typeid() { get "$A" "/api/v1/households/$HID/taxonomy" | python3 -c "
import sys, json
for c in json.load(sys.stdin):
    for t in c['types']:
        if t['code'] == '$1': print(t['id']); break"; }

inst() { get "$A" "/api/v1/households/$HID/institutions?q=$1" | python3 -c "
import sys, json
d = json.load(sys.stdin)
print(d[0]['id'] if d else '')"; }

echo "${BOLD}Adding accounts…${OFF}"
SBI=$(post "$A" "/api/v1/households/$HID/accounts" "{
...
ZERODHA=$(post "$A" "/api/v1/households/$HID/accounts" "{
...
add() { post "$A" "/api/v1/households/$HID/investments" "$1" >/dev/null; }
...
FLAT=$(post "$A" "/api/v1/households/$HID/investments" "{
...
post "$A" "/api/v1/households/$HID/liabilities" "{
