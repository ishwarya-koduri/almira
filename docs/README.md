...
| — | [Going live](../GO-LIVE.md) | The checklist per provider — DigiLocker, Account Aggregator, iOS/Android push, real SMS, WhatsApp, email: the interface contract, what the partner must supply, the steps and smoke test to flip it, and what is **not watched failing** (all of them, today). |
...
| 21 | [Still true?](21-still-true.md) | Records nobody has confirmed in a while come back to their owners: the per-type periods and the argument for each, what counts as confirmation (a yes or a value refresh, never an edit or a share), who is asked, snooze, the count-only nudge that does not nag, and the sweep that must run on the owner connection with each person's identity set. |
| 22 | [Handover readiness](22-handover-readiness.md) | One number for what you would hand over, with every gap named: the four checks (nominee, scan, sealed location, a trusted contact who can actually ask) and why each weighs the same, which holdings each applies to, why the score rounds down and is null when the data has not earned it, why it is computed as each viewer, and why "still true?" does not move it. |
| 23 | [Privacy notice](23-privacy-notice.md) | The notice the app shows from Settings and onboarding, still a draft pending legal review: what is stored, what the server can and cannot read, and the key-holder paragraph — it names another person, it is sealed, write a role not a name, and consent is with the lawyers. |
